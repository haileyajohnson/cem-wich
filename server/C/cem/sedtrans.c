...
double GetTransportVolumePotential (double alpha, double wave_height);
...
void WaveTransformation(struct BeachGrid *grid, double wave_angle, double wave_period, double wave_height)
{
    ...
    do
    {
        ...
        curr->transport_potential = GetTransportVolumePotential(local_alpha, local_wave_height);
...
double GetTransportVolumePotential (double alpha, double wave_height)
{
    ...
    return fabs(1.1 * rho * powf(GRAVITY, 3.0/2.0) * powf(wave_height, 5.0/2.0) * cos(alpha) * sin (alpha) * c_timestep_length);
}
...
{
    double cell_area = powf(grid->cell_size, 2);
    struct BeachNode *curr = grid->shoreline;
    ...
    do {
        ...
        double depth = GetDepthOfClosure(curr, ref_pos, ref_depth, shelf_slope, shoreface_slope, shore_angle, min_depth, grid->cell_size);
...
double GetDepthOfClosure(struct BeachNode *node, int ref_pos, double shelf_depth_at_ref_pos, double shelf_slope, double shoreface_slope, double shore_angle, double min_shelf_depth_at_closure, int cell_size)
{
    int x = node->row;
    // Eq 1
    double local_shelf_depth = shelf_depth_at_ref_pos + ((x - ref_pos) * cell_size * shelf_slope);

    // Eq 2
    double cross_shore_distance_to_closure = local_shelf_depth / (shoreface_slope - (cos(shore_angle)*shelf_slope));
    
    // Eq 3
    double cross_shore_pos_of_closure = x + cos(shore_angle) * cross_shore_distance_to_closure / cell_size;

    // Eq 4
    double closure_depth = shelf_depth_at_ref_pos + (cross_shore_pos_of_closure - ref_pos) * cell_size * shelf_slope;
