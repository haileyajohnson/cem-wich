...
const char* c_log_file = "../../test/output/CEM_LOG";
		...
		for (r = 0; r < c_rows; r++) {
			...
			if (IsLandCell(r, c)) {
				...
				while (r >= 0 && r < g_beachGrid.rows && c >= 0 && c < g_beachGrid.cols)
				{
					...
					int next_r = r + dir_r;
					int next_c = c + dir_c;
					while (!IsLandCell(next_r, next_c))
					{
...
void ReadConfigFile() {
	// open file
	char read_file_name[] = "server/test/config.txt";
...
double** SetupCells()
{
	...
	{
		FILE* file = fopen("server/test/inlet_test.txt", "r");
		if (file != NULL)
		{
			char line[2048];
			char* ptr;
