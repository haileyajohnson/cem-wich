			...
			/* This will define TotalBeachCells for this time step */
{
	...
	while ((Y[z] < Y_MAX-1) && (z < MaxBeachLength - 1)) {
		z++;
		NextX = -2;
		NextY = -2;
						...
						/* revised 5/04 LMV */
{
 ...
 /* calculate effective depth */
	...
	for (i = TotalBeachCells - 2; i >= 1; i--) { /*get the volumes across left side of cell for L cells */
	...
	ActualVolumeAcross[0] = ActualVolumeAcross[1];
	ActualVolumeAcross[TotalBeachCells - 1] = ActualVolumeAcross[TotalBeachCells - 2];
}
...
/* LMV rock fix - do not fix bare bedrock cells */
{
	...
	for (x = 1; x < ShadowXMax - 1; x++) {
		for (i = 1; i < Y_MAX-1; i++) { /* RCL: changing loops to ignore border */
			if (sweepsign == 1)
				y = i;
			else
				y = Y_MAX - i;
	 ...
	 type_of_rock[][]  */
{
	...
	for (x = (X_MAX - 1); x >= 0; x--) {
		for (y = 0; y < Y_MAX; y++) {
			PercentFullSand[x][y] = myConfig.grid[x][y];
		}
	}
}
